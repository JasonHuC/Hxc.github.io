'use server';
// 处理 照片管理 的服务器端操作，包括上传、更新、删除、标签管理、缓存同步、模糊图像生成、以及与 AI 图像处理 相关的功能。
// 所有这些操作都通过 runAuthenticatedAdminServerAction 函数执行，确保只有经过身份验证的管理员才能执行这些操作。
import {
  deletePhoto,
  insertPhoto,
...
import { deleteFile } from '@/features/gallery/services/storage';
import {
  getPhotosCached,
  getPhotosMetaCached,
...
import { TAG_FAVS, isTagFavs } from '@/features/gallery/tag';
import { convertPhotoToPhotoDbInsert } from './index';
import { runAuthenticatedAdminServerAction } from 'src/auth';
import { AI_IMAGE_QUERIES, AiImageQuery } from './ai';
import { streamOpenAiImageQuery } from '@/features/gallery/services/openai';
import {
...
import { UrlAddStatus } from '@/app/admin/gallery/AdminUploadsClient';
import { convertStringToArray } from '@/features/gallery/utility/string';
