...
export const getBlogs = async (params:GetBlogsDTO)=>{
    ...
    // 如果没有权限 或者 result.data.published是boolean
    if(published || !isUndefined(result.data.published)){
        const searchPublished : boolean | undefined = PUBLISHED_MAP[result.data.published!];//非空断言运算符。这个运算符用来告诉 TypeScript 编译器该表达式的结果不可能是 null 或 undefined。
